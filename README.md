...
- **Built-in Tools**: Read, Write, Shell, Glob, Grep, Todo — `BuiltInTools.GetAll(workingDirectory)`. A host hooks file writes (snapshots, auditing, policy) with `IFileWriteInterceptor`: `BuiltInTools.GetAll(workingDirectory, interceptor)`
